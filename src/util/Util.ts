export function generate<T>(fn: (i: number) => T, n: number, n2?: number) {
	...
	for (let i = start; i < end; i++) result.push(fn(i));

	return result;
}

export function remove<T>(array: T[], filter: (v: T) => boolean) {
	let n = 0;
...
export function shellSimilarity(a: string, b: string) {
	let common = commonPrefixLength(a, b) + commonSuffixLength(a, b);
	return common / Math.max(a.length, b.length);
}

export function oxfordCommaOr(array: string[]) {
	if (array.length == 1) return array[0];

	let result = "";
	for (let i = 0; i < array.length; i++) {
		if (i == array.length - 1) result += ", or ";
		else if (i > 0) result += ", ";
		result += array[i];
	}
	return result;
