...
export function findNoteOnFretboard(
	...
	for (let str = 0; str < tuning.length; str++) {
		...
		const minFretOctave =
			strOctave === undefined
				? undefined
				: strOctave + Math.floor(minFret / 12);
		...
		while (fret <= maxFret) {
			if (
				noteOctave === octave ||
				noteOctave === undefined ||
				octave === undefined
			) {
				let noteName: string;
				if (octave === undefined)
					noteName = Note.pitchClass(Note.fromMidiSharps(noteChroma));
				else noteName = Note.fromMidiSharps(noteChroma + 12 * octave);
...
export function noteAt(tuning: string[], string: number, fret: number) {
	return Note.fromMidiSharps(Note.midi(tuning[string])! + fret);
}

export function isSharpOrFlat(tuning: string[], string: number, fret: number) {
...
export function fretboardPos(fret: number, linear = false) {
	//< Computes the position on the fretboard (with scale length = 1)
	if (linear) return fret;
	return 1 - Math.pow(fretPositionFactor, fret);
}
export function invFretboardPos(x: number, linear = false) {
	if (linear) return x;
